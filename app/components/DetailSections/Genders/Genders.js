...
export default class Genders extends Component {
  ...
  toggleAvgLive = () => {
    // Fadeout description
    Animated.timing(this.textScale, {
      toValue: 0,
      duration: 100,
    }).start(() => {
      // timing callback
  ...
  animateBars = (toValueMen, toValueWomen, flag) => {
    Animated.parallel([
      Animated.timing(this.menBarWidth, {
        toValue: toValueMen,
        duration: 500,
      }),
      Animated.timing(this.womenBarWidth, {
  ...
  render() {
    if (this.state.showing === 'Average') {
      ...
      this.description =
        ...
        ' men and ' +
        this.numWomen +
        ' women on visiting right now';
    }

    return (
      <View style={styles.container}>
        <Text style={styles.heading}> People Here </Text>
        <Animated.Text
          style={[styles.description, {transform: [{scaleY: this.textScale}]}]}>
          {this.description}
        </Animated.Text>
        ...
        <View style={styles.peopleVisualContainer}>
          <View>
            <Image style={styles.genderIcon} source={manIcon} />
            <Text style={styles.menVisualText}>{this.numMen}</Text>
          </View>
          ...
          <View>
            <Image style={styles.genderIcon} source={womanIcon} />
            <Text style={styles.womenVisualText}>{this.numWomen}</Text>
          </View>
        </View>
        ...
        <TouchableOpacity
          ...
          <Text
            style={[
              styles.toggle,
              this.state.showing === 'Average' && styles.active,
            ]}>
            Here on average
          </Text>
          ...
          <Text
            style={[
              styles.toggle,
              this.state.showing === 'Live' && styles.active,
            ]}>
            Here right now
          </Text>
        </TouchableOpacity>
